		...
		<form method="post">
			...
			<table class="form-table optiontable editform" cellspacing="2" cellpadding="5">
				<tr valign="top">
					...
					<td width="60%">
						<input type="checkbox" id="username_managed" name="headers[username][managed]" checked="checked" disabled="true" <?php disabled( $shib_headers_constant ); ?>/> <?php _e( 'Managed', 'shibboleth' ); ?>
					</td>
				</tr>
				<tr valign="top">
					...
					<td>
						...
						disabled( $shib_headers_constant ); ?>/> <?php _e( 'Managed', 'shibboleth' ); ?>
					</td>
				</tr>
				<tr valign="top">
				...
				<tr>
					...
					<td>
						...
						?>>
							...
							<?php
							foreach ( $wp_roles->role_names as $key => $name ) {
								echo '<option value="' . $key . '"' . selected( $default_role, $key ) . '>' . __( $name ) . '</option>';
							}
							?>
						</select>
